...
export default function Aurora(props) {
  ...
  useEffect(() => {
    ...
    const getColor = hex => {
      if (!colorCache.has(hex)) { const c = new Color(hex); colorCache.set(hex, [c.r, c.g, c.b]); }
    ...
    const loop = t => {
      ...
      program.uniforms.uTime.value = (t - t0) * 0.001 * speed * 0.1;
      program.uniforms.uAmplitude.value = propsRef.current.amplitude ?? 1.0;
      program.uniforms.uBlend.value = propsRef.current.blend ?? blend;
      const stops = propsRef.current.colorStops ?? colorStops;
      program.uniforms.uColorStops.value = stops.map(getColor);
